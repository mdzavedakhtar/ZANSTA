      ...
      <AnimatePresence>
        {!isExiting && showLogo && (
          <motion.div
            ...
            className="space-y-6 relative will-change-transform"
          >
            {/* Background Ambient Crimson Glow Pulse */}
            <div className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-72 h-72 sm:w-96 sm:h-96 bg-[#8B0D1A]/25 rounded-full ${isMobile ? 'blur-3xl' : 'blur-[100px]'} pointer-events-none`} />

            {/* Wordmark with Rich Crimson Red & Off-White Gradient */}
            <div className="relative z-10 flex items-center justify-center gap-2 sm:gap-3">
              <h1 className="text-5xl sm:text-7xl md:text-8xl font-black font-display tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-[#F5F2ED] via-[#D92638] to-[#8B0D1A] select-none drop-shadow-[0_0_30px_rgba(139,13,26,0.7)]">
                ZANSTA
              </h1>
            ...
            {showTagline && (
              <motion.div
                initial={{ opacity: 0, y: 12, filter: isMobile ? 'none' : 'blur(6px)' }}
